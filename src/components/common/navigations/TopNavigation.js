...
const TopNavigation = ({ className }) => {
  ...
  const setActiveNav = (index) => {
    ...
    return 'text-[#5798F7] dark:text-primary block';
  };
  return (
    <nav className={` ${className}`}>
      <div className="max-w-full mx-auto mt-10 flex items-center gap-5">
        ...
        <div className="px-3 py-6 w-full flex flex-row relative justify-between">
          {navList.map((navItem, index) => (
            <div className="relative" key={index}>
                    ...
                    {navItem.menu.url !== '' ? (
                      <NavLink
                        className={({ isActive, isPending }) =>
                          isPending ? 'block' : isActive ? setActiveNav(index) : 'block'
                        }
                        to={navItem.menu.url}>
                        {navItem.menu.title}
                      </NavLink>
              ...
              <div
                className={`transition-all duration-300 absolute bg-white dark:bg-dark dark:text-white input-accent-shadow ${
                ...
                <ul className="text-base text-[#878787] space-y-1.5 ">
                  {navItem?.submenu?.length > 0 &&
                    navItem?.submenu?.map((submenu, i) => (
                      <li
                        key={i}
                        className={`text-[17px] dark:text-white w-max ${
